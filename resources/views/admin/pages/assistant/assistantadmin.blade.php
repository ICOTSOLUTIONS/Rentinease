                                        ...
                                        <td>
                                            <div class="row">
                                                <div class="col-md-12 d-flex justify-content-around">
                                                    <a href="{{ route('assistant.show',['assistant'=>$assistant->id]) }}" class="btn btn-info text-white"><i class="fas fa-eye"></i></a>
                                                    <a href="{{ route('assistant.edit', ['assistant' => $assistant->id]) }}" class="btn btn-success text-white"><i class="fas fa-edit"></i></a>
                                                    {{-- @if ($assistant->id != 1 && $assistant->fname != 'superadmin') --}}
                                                    <form action="{{ route('assistant.destroy', ['assistant' => $assistant->id]) }}" method="POST">
                                                        @method('DELETE')
                                                        @csrf
